...
Screen::Screen(const std::string screenFileName) :
	_screen(nullptr),
	_screenWidth(0),
	_screenHight(0),
	_timeLimit(0),
...
void Screen::Draw(IPainter& painter) const {
	DrawWalls(painter);
	DrawExits(painter);
	DrawBlocks(painter);
	DrawShips(painter);
...
void Screen::PhysicsUpdate(float elapsedTime, const IController& controller) {
...
void Screen::Update(float elapsedTime, const IController& controller) {
	if (_state != ScreenState::Playing) {
		return;
	}
...
int Screen::Hight() const {
	return _screenHight;
}

int Screen::StartingLifePoints() const {
	return _lifePoints;
}

int Screen::SecondsLeftToFinishScreen() const {
...
void Screen::GroundShips() {
	for (auto& ship : _ships) {
		ship.Stop();
	}
}

Point Screen::ActiveShipMainPoint() {
...
char Screen::ActiveShipName() const {
	return _activeShipName;
}

const std::vector<GameObject*>& Screen::Entities() const {
	return _entities;
}

std::vector<GameObject*>& Screen::Entities() {
	return _entities;
}

bool Screen::IsWall(const Point& point) const {
	auto col = point.X;
...
Point Screen::ClipToScreen(const Point& point) const {
	auto ret = point;
	if (point.X < 0)
	{
...
void Screen::LoadScreenCharsFromFile(const std::string screenFileName) {
...
void Screen::CreateBlocksFromScreenChars() {
	if (!IsLoaded()) {
		return;
	}
...
void Screen::CreateShipsFromScreenChars() {
	if (!IsLoaded()) {
		return;
	}
...
void Screen::CreateExitsFromScreenChars() {
	if (!IsLoaded()) {
		return;
	}
...
void Screen::DrawWalls(IPainter& painter) const {
	for (auto row = 0; row < _screenHight; row++)
	{
		for (auto col = 0; col < _screenWidth; col++)
		{
...
void Screen::DrawExits(IPainter& painter) const {
	for (const auto& exit : _exits)
	{
		exit.Draw(painter);
	}
...
void Screen::DrawBlocks(IPainter& painter) const {
...
void Screen::DrawShips(IPainter& painter) const {
	for (const auto& ship : _ships)
	{
		ship.Draw(painter);
	}
...
void Screen::ReEnterEntitiesInOrder() {
	_entities.clear();
...
void Screen::SwapShipsOnInput(const IController& controller) {
...
void Screen::SwitchToShip(const char name) {
	for (auto& ship : _ships) {
		if (ship.Is(name)) {
			ship.Activete();
		}
...
bool Screen::ShipInsideExit(Ship& ship, Exit& exit) const {
...
void Screen::RemoveShipsInExit(Exit& exit) {
